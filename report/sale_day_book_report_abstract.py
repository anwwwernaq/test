...
class SalesDayBookReportAbstract(models.AbstractModel):
    _name = 'report.bi_all_in_one_sale_reports.sales_daybook_report_document'
    _description = 'Sale Day Book Report Abstract'


    def _get_report_values(self, docids, data=None):
        docs = self.env['sale.day.book.report'].browse(docids)

        data  = { 'start_date': docs.start_date, 'end_date': docs.end_date}

        return {
                   'doc_ids': self.ids,
                   'doc_model': 'sale.day.book.report',
                   'docs': docs,
                   'data' : data,
    ...
    def _get_data(self,obj):
        ...
        if obj.amount_total > 0:
          ...
          if obj.name[0:3] == 'INV':
            ...
            for items in obj.amount_by_group:
              final_dict['tax'] += items[1]
    ...
    def _get_total_tax(self):
      lst = list(ids.id for ids in self.env['sale.day.book.report'].search([]))
      start_date = self.env['sale.day.book.report'].browse(max(lst)).start_date
      end_date = self.env['sale.day.book.report'].browse(max(lst)).end_date
