...
class CustomerInvoicePaymentReport(models.TransientModel):
	_name = 'customer.invoice.payment.report'
	...
	def default_get(self, fields):
		...
		rec = super(CustomerInvoicePaymentReport, self).default_get(fields)
	...
	def invoice_payment_pdf_report(self):
		...
		elif self.select_state == 'open':
			list1.extend(['not_paid','in_payment','partial'])
		...
		if len(self.user_ids.ids) == 0:
			...
			sale_orders = self.env['sale.order'].search([('date_order', '>=', self.start_date), ('date_order', '<=', self.end_date),
														 ('company_id', 'in', selected_companies),  ('user_id', '=', self.user_id.id)])

		else:
			sale_orders = self.env['sale.order'].search([('date_order', '>=', self.start_date), ('date_order', '<=', self.end_date),
														 ('company_id', 'in', selected_companies),  ('user_id', 'in', self.user_ids.ids)])
		...
		data1 = self.env.ref(
			'bi_all_in_one_sale_reports.customer_invoice_payment_report_action').report_action(self, data=data2)
		return data1
	...
	def invoice_payment_xls_report(self):
		...
		row = 5
		sale_orders = self.env['sale.order'].search([('date_order', '>=', self.start_date), ('date_order', '<=', self.end_date),
													 ('company_id', 'in', selected_companies),  ('user_id', 'in', self.user_ids.ids), ('state', '=', 'sale')])
		list1 = []
		if self.select_state == 'all':
			list1.extend(['draft','not_paid', 'paid', 'in_payment','partial'])
		elif self.select_state == 'open':
					...
					for invoice in invoice_payments:
						if invoice.invoice_payments_widget !='false':
							...
							for i in res['content']:
								...
								if i['journal_name'] == 'Cash':
									cash_amount += i['amount']
							if invoice.reversed_entry_id.state=='posted' and invoice.name!='/':
								row += 1
								...
								worksheet.write(row, 1, invoice.invoice_date.strftime("%d/%m/%Y"))
		...
		action = {
			...
			'name': 'contract',
			'url': '/web/content/customer.invoice.payment.report/%s/data/Payment Report for Invoice_Sales.xls?download=true' % (self.id),
			'target': 'self',
		}
		return action
