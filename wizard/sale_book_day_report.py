...
class SaleDayBookReport(models.TransientModel):
	_name = "sale.day.book.report"
	_description = 'Sale Day Book Report'
	...
	def sales_day_book_pdf_report(self):
		datas = {
			'ids': self._ids,
			'model': 'sale.day.book.report',
			'start_date': self.start_date,
			'end_date': self.end_date
		}
		return self.env.ref('bi_all_in_one_sale_reports.sales_day_book_report_template').report_action(self)


	def sales_day_book_xls_report(self):
		filename = 'Sale Day Book Report.xls'
		...
		worksheet.write_merge(0, 1, 0, 6, "Sales Day Book Report", style=heading_style)
		...
		header_tmp = self.env['report.bi_all_in_one_sale_reports.sales_daybook_report_document']._get_ref()
		...
		for invoice in invoice_ids:
			...
			detail_data = self.env['report.bi_all_in_one_sale_reports.sales_daybook_report_document']._get_data(invoice)
		...
		row += 1
		total_categ = self.env['report.bi_all_in_one_sale_reports.sales_daybook_report_document']._get_total(obj)
		...
		self.write({
			...
			'file_name': 'Sale Day Book Report.xls'
		})
		action = {
			'type': 'ir.actions.act_url',
			'name': 'contract',
			'url': '/web/content/sale.day.book.report/%s/data/Sale Day Book Report.xls?download=true' % (
				self.id),
			'target': 'self',
		}
		return action
